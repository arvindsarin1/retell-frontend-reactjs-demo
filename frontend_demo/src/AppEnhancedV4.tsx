...
const AppEnhancedV4 = () => {
  ...
  // Load LLM configuration from backend on mount
  useEffect(() => {
  ...
  const loadLLMConfiguration = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/llm-config');
      if (response.ok) {
  ...
  const saveLLMConfiguration = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setOriginalPrompt(llmConfig.systemPrompt);
        // Update agent configuration for live calls
  ...
  async function registerCall(agentId: string): Promise<RegisterCallResponse> {
    try {
      const response = await fetch("http://localhost:8080/create-web-call", {
        ...
        body: JSON.stringify({
          agent_id: agentId,
          // Pass the current system prompt to be used in the call
        ...
        <div className="settings-modal">
          <div className="settings-content">
            ...
            <div className="settings-body">
              {isLoadingConfig ? (
                <div className="loading-state">Loading configuration from llm_config.py...</div>
              ) : (
                <>
                  <div className="form-group">
                    ...
                    <p className="help-text">
                      This prompt is loaded from and saved to <code>llm_config.py</code>. 
                      Changes will be applied to live conversations immediately after saving.
                    </p>
                  </div>
                  ...
                  <div className="settings-actions">
                    ...
                    >
                      {isSavingConfig ? 'Saving...' : 'Save to llm_config.py'}
                    </button>
                    <button 
